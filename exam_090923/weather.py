...
connection = sqlite3.connect("exam.sl3", 5)
...
resp = requests.get("https://ua.sinoptik.ua/погода-святопетрівське")
...
if resp.status_code == 200:
    soup = BeautifulSoup(resp.text, features = "html.parser")
    soup_list = soup.find_all('p')
    soup_list = str(soup_list[2]).split('p class="day-link" data-link="//ua.sinoptik.ua/погода-святопетрівське/')
